...
// FIXME: stop using it
#include "exclamation/cuda/Float8_e4m3fn.cuh"
...
tensor_convert(
  ...
  if constexpr (len % 4 == 0) {
    ...
    const auto scale4 = recast<exclamation::half2x2>(scale);
    const auto bias4 = recast<exclamation::half2x2>(bias);
    auto out4 = recast<exclamation::half2x2>(out);

    CUTE_UNROLL
    for (int i = 0; i < size(in4); i++) {
      auto v = fast_type_convert<exclamation::half2x2>(in4(i));
      auto a = scale4(i);
      auto b = bias4(i);
