...
#include "exclamation/cuda/atomics.cuh"
#include "exclamation/cuda/paged_attention/lbp_attention_task_selector.cuh"
...
__global__ void
EXCLAMATION_LAUNCH_BOUNDS(NumThreads, PAGED_ATTENTION_MAXNREG) lbp_attention_data_parallel_kernel(
...
#if !defined(EXCLAMATION_SPLIT_COMPILATION) || defined(PAGED_ATTENTION_KERNEL_IMPL)
{
  ...
  // NOTE: nvcc bug?
  //  error: identifier "exclamation::TaskConfig<(int)512> ::TaskChunkSeqLen" is undefined in device code
  constexpr int TaskChunkSeqLen = Config::TaskChunkSeqLen;
